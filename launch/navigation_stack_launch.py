...
def generate_launch_description():
    ...
    wheel_radius_arg = DeclareLaunchArgument(
        ...
        description="Wheel radius of the robot (meters)",
    )

    wheel_base_arg = DeclareLaunchArgument(
        "wheel_base",
        default_value="0.07",
        description="Wheel base of the robot (meters)",
    )

    params_file_arg = DeclareLaunchArgument(
        "params_file",
        default_value=PathJoinSubstitution([my_pkg_dir, "config", "nav2_params.yaml"]),
    ...
    slam_toolbox_node = Node(
        ...
        parameters=[
            ...
            {"use_sim_time": False, "base_frame": "base_link", "odom_frame": "odom"},
        ],
    )
