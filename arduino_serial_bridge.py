...
class ArduinoBridge(Node):
    def __init__(self):
        ...
        # Parameters
        default_port = (
            "/dev/arduino" if os.path.exists("/dev/arduino") else "/dev/ttyAS4"
        )
        self.declare_parameter("port", default_port)
        self.declare_parameter("baudrate", 115200)
        self.declare_parameter(
            "wheel_radius", 0.0325
        )  # meters (radius roda) - matching Arduino WHEEL_DIAMETER = 6.5 cm
        self.declare_parameter(
            "wheel_base", 0.07
        )  # meters (jarak antar roda) — HARUS sama dengan Arduino
    ...
    def _send_vel_command(self, msg: Twist):
        ...
        if command == "CMD,DIAM\n":
            ...
            rpm_changed = (
                self.last_sent_rpm is None
                or abs(rounded_rpm - self.last_sent_rpm) >= 0.5
            )
        ...
        with self.serial_lock:
            ...
            try:
                ...
                self.get_logger().info(
                    f"Connected to Arduino on {self.port} at {self.baudrate} baud."
                )
                return True
    ...
    def serial_read_loop(self):
        ...
        while rclpy.ok():
            ...
            try:
                ...
                if line.startswith("ODOM,"):
                    ...
                    if len(parts) == 6:
                        try:
                            ...
                            self.process_direct_odometry(
                                odom_x, odom_y, odom_theta, rpm_kiri, rpm_kanan
                            )
                            error_count = 0
                        except ValueError as e:
                            self.get_logger().warn(
                                f"Failed to parse ODOM line '{line}': {e}"
                            )
                    ...
                    if len(parts) == 3:
                        ...
                        password = parts[2]
                        self.get_logger().info(
                            f"Received WiFi connection request from ESP32: SSID={ssid}"
                        )
                ...
                error_count += 1
                self.get_logger().warn(
                    f"Error reading from serial (attempt {error_count}/{MAX_ERRORS}): {e}"
                )
                if error_count >= MAX_ERRORS:
                    self.get_logger().error(
                        f"Max serial errors reached ({MAX_ERRORS}). Disconnecting and reconnecting..."
                    )
                    with self.serial_lock:
                        self.connected = False
                        if self.ser is not None:
    ...
    def change_wifi(self, ssid, password):
        ...
        def run_nmcli():
            ...
            try:
                ...
                if res.returncode == 0:
                    self.get_logger().info(
                        f"Successfully connected to WiFi SSID: '{ssid}'"
                    )
                else:
                    self.get_logger().error(
                        f"Failed to connect to WiFi SSID: '{ssid}'. Error: {res.stderr.strip()}"
                    )
            except subprocess.TimeoutExpired:
                self.get_logger().error(
                    f"Timeout expired while connecting to WiFi SSID: '{ssid}'"
                )
            except Exception as e:
