...
class RobotFirebaseBridge(Node):
    ...
    def _init_firebase(self) -> bool:
        try:
            ...
            paths_to_check = [
                os.path.join(
                    script_dir, cred_filename
                ),  # Lokasi di install space / script dir
                os.path.join(
                    os.getcwd(), cred_filename
                ),  # Lokasi di root workspace saat ini
                os.path.join(
                    os.path.expanduser("~"), cred_filename
                ),  # Lokasi di home directory
            ]
    ...
    def odom_callback(self, msg: Odometry):
        ...
        if dt >= self.odom_publish_interval:
            ...
            R = (
                self.R
            )  # wheel radius (meters) - matching Arduino WHEEL_DIAMETER = 6.5 cm
            L = self.L  # wheel base (meters)
