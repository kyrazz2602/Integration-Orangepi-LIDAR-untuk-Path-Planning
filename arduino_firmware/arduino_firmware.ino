...
// ============================================
void encoderKananA() {
  if (digitalRead(ENC_KANAN_A) == digitalRead(ENC_KANAN_B)) pulseKanan--;
  else pulseKanan++;
}
void encoderKananB() {
  if (digitalRead(ENC_KANAN_A) == digitalRead(ENC_KANAN_B)) pulseKanan++;
...
void encoderKiriA() {
  if (digitalRead(ENC_KIRI_A) == digitalRead(ENC_KIRI_B)) pulseKiri--;
  else pulseKiri++;
}
void encoderKiriB() {
  if (digitalRead(ENC_KIRI_A) == digitalRead(ENC_KIRI_B)) pulseKiri++;
...
void eksekusiPerintah(String cmd) {
  ...
  if (cmd.startsWith("CMD,VEL,")) {
    ...
    if (val > 0 && val <= 200) {
      targetRPM = val;
      Serial2.print("ACK:RPM:"); Serial2.println(targetRPM, 0);
    }
  }
...
void bacaSensorHalangan() {
  ...
  Serial2.print(jarakDepan, 1); Serial2.print(",");
  Serial2.print(jarakKiriS, 1); Serial2.print(",");
  Serial2.print(jarakKananS,1); Serial2.print(",");
  Serial2.print(irKiri    ? 1 : 0); Serial2.print(",");
  Serial2.print(irTengah  ? 1 : 0); Serial2.print(",");
...
void setup() {
  ...
  pinMode(ENC_KANAN_A, INPUT_PULLUP); pinMode(ENC_KANAN_B, INPUT_PULLUP);
  pinMode(ENC_KIRI_A,  INPUT_PULLUP); pinMode(ENC_KIRI_B,  INPUT_PULLUP);
...
void loop() {
  ...
  if (now - lastPrint >= intervalPrint) {
    ...
    Serial.print("HCSR Depan:"); Serial.print(jarakDepan, 1); Serial.print("cm");
    Serial.print(blockedDepan() ? "(BLOCK)" : "(OK)");
    Serial.print(" | Kiri:"); Serial.print(jarakKiriS, 1); Serial.print("cm");
    Serial.print(blockedKiri() ? "(BLOCK)" : "(OK)");
    Serial.print(" | Kanan:"); Serial.print(jarakKananS, 1); Serial.print("cm");
